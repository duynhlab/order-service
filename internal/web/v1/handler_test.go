...
func newHandler(repo domain.OrderRepository) *OrderHandler {
	return NewOrderHandler(logicv1.NewOrderService(repo, nil, nil), nil, nil, nil, "", nil, "")
}
...
func TestGetOrderDetails_WithShipment(t *testing.T) {
	...
	h := NewOrderHandler(logicv1.NewOrderService(repo, nil, nil), nil, ship, nil, "", nil, "")
	c, rec := newCtx(http.MethodGet, "/order/v1/private/orders/1/details", "user1", gin.Params{{Key: "id", Value: "1"}})
	h.GetOrderDetails(c)
...
func TestCreateOrder_CartEmpty(t *testing.T) {
	...
	defer srv.Close()
	h := NewOrderHandler(logicv1.NewOrderService(&mockOrderRepo{}, nil, nil), NewCartClient(srv.URL), nil, nil, "", nil, "")
	c, rec := ctxWithBody(http.MethodPost, "/order/v1/private/orders", "user1", "{}", nil)
	h.CreateOrder(c)
	if rec.Code != http.StatusBadRequest {
...
func TestGetOrderDetails_WithPayment(t *testing.T) {
	...
	h := NewOrderHandler(logicv1.NewOrderService(repo, nil, nil), nil, nil, nil, "", pay, "")
	c, rec := newCtx(http.MethodGet, "/order/v1/private/orders/1/details", "user1", gin.Params{{Key: "id", Value: "1"}})
	h.GetOrderDetails(c)
